	 ...
	 * the highest index of an element's occurrence in the array.
	 * The solution is an iterative one with Time Complexity O(log n)
	 * and Space Complexity of O(1).
	 */
	public int getHighestIndexBinarySearch(int[] a, int k)
	{
		if(a==null)
		...
		while(l<=r)
		{
			...
			if(a[m]==k) //condition is a bit tricky here, could easily run into out of index exceptions if not careful
			{
				if(m==a.length-1 || a[m+1]!=k)
				   return m;
			}
	 ...
	 * iterate the array in the forward direction. Such a solution would
